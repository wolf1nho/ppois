...
Locker::Locker(const char gender, const unsigned num)
{
    ...
    bool damaged = false;          // поврежден ли
    bool has_ventilation = false;  // есть ли вентиляция
    bool has_mirror = false;       // есть ли зеркало на дверце
    std::string material = "wood"; // материал: "metal", "plastic", "wood"
    std::string color = "brown";
}

Locker::Locker(PassKey *passkey, const unsigned num, const char gender, const unsigned capacity, const bool damaged,
               const bool has_ventilation, const bool has_mirror, const std::string material, const std::string color)
{
    locked = true;
    linked = false;
    this->num = num;
